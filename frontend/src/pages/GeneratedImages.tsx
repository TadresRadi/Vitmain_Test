...
import { Facebook, Edit3, Send, CheckCircle2, Loader2, MousePointerSquareDashed } from "lucide-react"
...
export default function GeneratedImages() {
  ...
  useEffect(() => {
    api.get("/images/mock-posts").then((res) => {
      ...
      <div className="space-y-6 pb-20">
        <div className="flex justify-between items-end">
          ...
          {selectedPosts.length > 0 && (
            ...
            >
              <span className="text-sm font-medium text-white">{selectedPosts.length} {t("campaignPortfolio.selected")}</span>
              <Button 
                size="sm" 
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {mockPosts.map((post) => (
              ...
              >
                <Card className={`glass-dark border transition-all duration-200 overflow-hidden ${
                  selectedPosts.includes(post.id) ? 'border-vitamin-base ring-1 ring-vitamin-base' : 'border-white/10 hover:border-white/30'
                }`}>
                  <div className="relative aspect-video">
                    <img src={post.previewDesign} alt="Design Preview" className="w-full h-full object-cover opacity-80" />
                    ...
                    <div className="absolute top-4 right-4">
                      <button 
                        ...
                        className={`w-6 h-6 rounded-md flex items-center justify-center transition-colors ${
                          selectedPosts.includes(post.id) ? 'bg-vitamin-base text-white' : 'bg-black/50 text-white/50 hover:bg-black/70'
                        }`}
                      >
                        {selectedPosts.includes(post.id) ? <CheckCircle2 className="w-4 h-4" /> : <MousePointerSquareDashed className="w-4 h-4" />}
                      </button>
                    </div>
                  </div>
                  ...
                  <CardContent className="p-6 space-y-4">
                    ...
                    <div className="bg-white/5 p-4 rounded-lg border border-white/5">
                      ...
                      <Badge variant="outline" className="text-vitamin-400 border-vitamin-400/30">CTA: {post.cta}</Badge>
                    </div>
                    ...
                    {editingPost === post.id ? (
                      ...
                      >
                        <label className="text-xs font-medium text-white/60 uppercase tracking-wider">{t("campaignPortfolio.requestModification")}</label>
                        <Textarea 
                        ...
                        <div className="flex justify-end gap-2">
                          <Button variant="ghost" size="sm" onClick={() => setEditingPost(null)} className="text-white/60 hover:text-white">{t("campaignPortfolio.cancel")}</Button>
                          <Button 
                            size="sm" 
                          ...
                          >
                            {isSubmitting ? t("campaignPortfolio.sending") : <><Send className="w-3 h-3 mr-2" /> {t("campaignPortfolio.sendRequest")}</>}
                          </Button>
                        </div>
                      </motion.div>
                    ) : (
                      <div className="flex justify-end border-t border-white/10 pt-4">
                        <Button variant="ghost" size="sm" className="text-white/60 hover:text-white" onClick={() => setEditingPost(post.id)}>

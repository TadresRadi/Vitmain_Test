...
export default function NewOnboarding() {
  ...
  const getQuestions = () => {
    ...
    if (
      ...
      businessTypeCategories[answers.businessType as keyof typeof businessTypeCategories]
  ...
  const handleNext = () => {
    ...
    if (question.id === 'businessType') {
      const subcategories =
        businessTypeCategories[currentAnswer as keyof typeof businessTypeCategories]
          ?.subcategories ?? []
  ...
  const completeOnboarding = async (finalAnswers: OnboardingFormData) => {
    ...
    try {
      ...
      toast({
        ...
        description: t(
          'onboarding.new.savedDesc',
          'Next, choose a plan to unlock your campaign posts.'
        ),
      })
      ...
      <div className="min-h-screen relative overflow-hidden">
        ...
        <div className="relative z-10 min-h-screen flex flex-col items-center justify-center px-4 text-center">
          ...
          <p className="text-white/60 max-w-md">
            {t(
              'onboarding.new.savingDesc',
              "Your answers are being saved. You'll pick a plan next."
            )}
          </p>
        </div>
      </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <h2 className="text-3xl font-bold text-white mb-8">
                  {questions[currentQuestion].id === 'businessName' &&
                    t('onboarding.new.businessName')}
                  {questions[currentQuestion].id === 'governorate' &&
                    t('onboarding.new.governorate')}
                  {questions[currentQuestion].id === 'businessType' &&
                    t('onboarding.new.businessType')}
                  {questions[currentQuestion].id === 'businessSubtype' &&
                    t('onboarding.new.businessSubtype')}
                  {questions[currentQuestion].id === 'marketingGoals' &&
                    t('onboarding.new.marketingGoalsLabel')}
                  {questions[currentQuestion].id === 'targetAudience' &&
                    t('onboarding.new.targetAudience')}
                  {questions[currentQuestion].id === 'toneOfVoice' &&
                    t('onboarding.new.toneOfVoice')}
                </h2>
                ...
                {showOtherInput ? (
                  ...
                  <div className="space-y-3">
                    {questions[currentQuestion].options?.map((option) => (
                      ...
                      >
                        ...
                        {selectedMarketingGoals.includes(option) && <Check className="h-5 w-5" />}
                      </motion.button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-3">
                    {questions[currentQuestion].options?.map((option) => (
                      ...
                      >
                        <span className="text-lg">
                          {questions[currentQuestion].id === 'businessType' &&
                            t(`onboarding.new.businessTypes.${option}`)}
                          {questions[currentQuestion].id === 'businessSubtype' &&
                            t(`onboarding.new.businessSubtypes.${option}`)}
                          {questions[currentQuestion].id === 'targetAudience' &&
                            t(`onboarding.new.targetAudiences.${option}`)}
                          {questions[currentQuestion].id === 'toneOfVoice' &&
                            t(`onboarding.new.toneOfVoices.${option}`)}
                        </span>
                      </motion.button>
                    ))}
                  </div>
                  ...
                  <Input
                    ...
                    placeholder={
                      questions[currentQuestion].id === 'businessName'
                        ? t('onboarding.new.businessNamePlaceholder')
                        : questions[currentQuestion].id === 'governorate'
                          ? t('onboarding.new.governoratePlaceholder')
                          : ''
                    }
                    className="bg-white/10 border-white/20 text-white placeholder-white/50 text-lg px-6 py-4 h-auto min-h-[60px]"
                    onKeyDown={(e) => {

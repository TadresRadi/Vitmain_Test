...
export default function UsersSection({ onOpenUserLogs }: UsersSectionProps) {
    ...
    try {
      ...
      alert(
        err.response?.data?.error ||
          t('adminDashboard.failedCreateSupervisor', 'Failed to create supervisor')
      )
    } finally {
  ...
  const handleDeleteUser = async (userId: string) => {
    if (!confirm(t('adminDashboard.confirmDelete', 'Are you sure you want to delete this user?')))
      return
    try {
              ...
              <form onSubmit={handleCreateSupervisor} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">
                    {t('adminDashboard.email', 'Email Address')}
                  </label>
                  <input
                    type="email"
                    required
                ...
                <div className="space-y-2">
                  <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">
                    {t('adminDashboard.password', 'Temporary Password')}
                  </label>
                  <input
                    type="password"
                    required
                        ...
                        <td className="p-4">
                          ...
                          >
                            ...
                            <option value="supervisor">
                              {t('adminDashboard.supervisor', 'Supervisor')}
                            </option>
                            <option value="super_admin">
                              {t('adminDashboard.admin', 'Super Admin')}
                            </option>
                          </select>
                        </td>

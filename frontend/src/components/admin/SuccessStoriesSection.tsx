        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">{t('adminDashboard.displayMode', "Display Mode")}</label>
              <select
                value={storySettings.mode}
                onChange={(e) => setStorySettings({ ...storySettings, mode: e.target.value as 'auto' | 'manual' })}
            ...
            <div className="space-y-2">
              <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">{t('adminDashboard.rotationInterval', "Rotation Interval (seconds)")}</label>
              <input
                type="number"
                min="5"
                max="300"
                value={storySettings.rotation_interval}
                onChange={(e) => setStorySettings({ ...storySettings, rotation_interval: parseInt(e.target.value) || 24 })}
                className="w-full bg-black/40 border border-white/10 text-white rounded-lg px-4 py-2.5 text-sm focus:border-red-500 focus:outline-none"
              />
            </div>
          </div>
          ...
          <Card className="bg-black/45 border-red-500/20 backdrop-blur-md sticky top-[170px]">
            <CardHeader className="border-b border-white/5 pb-4">
              <CardTitle className="text-lg font-bold text-white flex items-center gap-2">
                ...
                {editingSuccessStory ? t('adminDashboard.editSuccessStory', "Edit Success Story") : t('adminDashboard.addSuccessStory', "Add Success Story")}
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6">
              <form onSubmit={handleSaveSuccessStory} className="space-y-4">
                <div className="space-y-2">
                  <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">{t('adminDashboard.contentEn', "Content (English)")}</label>
                  <textarea
                    required
                    value={successStoryForm.content_en}
                    onChange={(e) => setSuccessStoryForm({ ...successStoryForm, content_en: e.target.value })}
                ...
                <div className="space-y-2">
                  <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">{t('adminDashboard.contentAr', "Content (Arabic)")}</label>
                  <textarea
                    required
                    value={successStoryForm.content_ar}
                    onChange={(e) => setSuccessStoryForm({ ...successStoryForm, content_ar: e.target.value })}
                ...
                <div className="space-y-2">
                  <label className="text-xs text-white/60 font-semibold uppercase tracking-wider">{t('adminDashboard.video', "Video File")}</label>
                  <input
                    type="file"
                    accept="video/*"
                    onChange={(e) => {
                  ...
                  {editingSuccessStory?.video_url && !successStoryForm.video && (
                    <p className="text-xs text-white/40 mt-1">{t('adminDashboard.currentVideo', "Current video:")} {editingSuccessStory.video_url}</p>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="success_story_active" className="text-xs text-white/60">{t('adminDashboard.active')}</label>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {savingSuccessStory ? (
                      ...
                      <>
                        <Save className="w-4 h-4 mr-2" />
                        {editingSuccessStory ? t('adminDashboard.update') : t('adminDashboard.create')}
                      </>
                    )}
                  </Button>
                  {editingSuccessStory && (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/5 bg-white/5 text-white/50 text-left font-semibold">
                      {storySettings.mode === 'manual' && <th className="p-4">{t('adminDashboard.featured', "Featured")}</th>}
                  ...
                  <tbody>
                        ...
                        {storySettings.mode === 'manual' && (
                          <td className="p-4">
                            <input
                              ...
                              onChange={() => setStorySettings({ ...storySettings, featured_video_id: story.id })}
                              className="w-4 h-4 bg-black/40 border border-white/10 rounded focus:border-red-500 focus:outline-none"
                            />
                          </td>
                        )}
                        <td className="p-4">
                          <div className="font-bold text-white line-clamp-2 max-w-[300px]">{story.content_en}</div>
                          <div className="text-xs text-white/40 mt-1">{new Date(story.created_at).toLocaleDateString()}</div>
                        </td>
                        <td className="p-4">
                          {story.video_url ? (
                            <div className="flex items-center gap-2">
                            ...
                            <span className="text-white/40 text-xs">{t('adminDashboard.noVideo', "No video")}</span>
                          )}
                        </td>
                        <td className="p-4">
                          <Badge className={story.is_active ? 'bg-green-500/20 border-green-500/30 text-green-400' : 'bg-red-500/20 border-red-500/30 text-red-400'}>
                            {story.is_active ? t('adminDashboard.active') : t('adminDashboard.inactive')}
                          </Badge>
                        </td>
                    ...
                    {successStories.length === 0 && (
                      <tr>
                        <td colSpan={storySettings.mode === 'manual' ? 5 : 4} className="p-12 text-center text-white/40">

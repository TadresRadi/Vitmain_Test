      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-extrabold text-white mt-2 font-sans tracking-tight">
              {value}
            </p>
          </div>
          <div
            className={`p-3 rounded-xl bg-white/5 group-hover:scale-105 transition-transform duration-300`}
          >
...
export default function OverviewSection({ data }: OverviewSectionProps) {
  ...
  if (!data)
    return <div className="text-white/40 text-xs py-10">Accessing platform statistics...</div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          title={t('adminDashboard.totalUsers', 'Total Explorers & Clients')}
          value={data.total_users}
          icon={Users}
          color="text-red-500"
        />
        <StatCard
          title={t('adminDashboard.totalPaymentsReceived', 'Total Payments Received')}
          value={data.total_payments || 0}
          icon={DollarSign}
          color="text-green-400"
        />
        <StatCard
          title={t('adminDashboard.postsGenerated', 'Premium Posts Generated')}
          value={data.total_posts}
          icon={MessageSquare}
          color="text-yellow-500"
        />
        <StatCard
          title={t('adminDashboard.imagesGenerated', 'Campaign Visual Mocks')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="bg-black/45 border-red-500/20 backdrop-blur-md overflow-hidden">
          <CardHeader className="border-b border-white/5 bg-white/5 p-4">
            <CardTitle className="text-sm font-bold text-white uppercase tracking-wider">
              {t('adminDashboard.userGrowth', 'User Enrollment Timeline')}
            </CardTitle>
          </CardHeader>
        ...
        <Card className="bg-black/45 border-red-500/20 backdrop-blur-md overflow-hidden">
          <CardHeader className="border-b border-white/5 bg-white/5 p-4">
            <CardTitle className="text-sm font-bold text-white uppercase tracking-wider">
              {t('adminDashboard.subscriptionDistribution', 'SaaS Subscription Distribution')}
            </CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <ResponsiveContainer width="100%" height={300}>
